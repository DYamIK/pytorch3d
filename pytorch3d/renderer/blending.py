...
class BlendParams(NamedTuple):
    ...
    成员变量：
        sigma (float): 在SoftmaxPhong中，控制用于计算基于2D距离概率的sigmoid函数宽度。
            决定形状边缘的锐利程度。数值越高 => 面片的边缘越模糊。
            在SplatterPhong中，此为高斯核的标准差。
            数值越高 => 溅射效果越强，渲染图像越模糊。
        gamma (float): 控制用于设置颜色不透明度的指数函数的缩放比例。
            数值越高 => 面片透明度越高。
        background_color: 背景色的RGB值，可以是一个元组或包含三个浮点数的张量。
    """
